...
create table public.deals (
    ...
    sales_id bigint,
    index smallint,
    supervisor_sales_id bigint,
    supervisor_lifeguard_id bigint,
    custom_fields jsonb not null default '{}'::jsonb,
    attachments jsonb not null default '[]'::jsonb,
    constraint deals_one_supervisor_check check (
...
create table public.deal_lifeguards (
    ...
    created_at timestamp with time zone not null default now(),
    updated_at timestamp with time zone not null default now(),
    constraint deal_lifeguards_status_check check (status in ('proposed', 'confirmed', 'on_site', 'declined', 'replaced'))
);
...
alter table public.custom_field_definitions
    add constraint custom_field_definitions_sales_id_fkey foreign key (sales_id) references public.sales(id);
...
create unique index custom_field_definitions_entity_key_idx on public.custom_field_definitions (entity_type, field_key);
create index custom_field_definitions_entity_active_order_idx on public.custom_field_definitions (entity_type, active, display_order);
